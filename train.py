...
if len(cfg.USING_GPU) > 1:
    divices = [int(i) for i in cfg.USING_GPU]
    model = nn.DataParallel(model, device_ids = divices)
else:
...
optimizer = optim.Adam(filter(lambda p: p.requires_grad, list(model.parameters())), lr=cfg.LR)
